...
to find-new-pile
  if pcolor != [color] of self
  [
    if [color] of self = green AND pcolor = yellow
    [get-away]

    if [color] of self = yellow AND pcolor = green
    [get-away]
...
to put-down-chip
  ifelse pcolor = black
  [
    ifelse [energy] of self > 0
    [
      let t_color [color] of self
      let t_inverse_color nobody

      ifelse t_color = yellow
      [
        set t_inverse_color green
      ]
      [
        set t_inverse_color yellow
      ]

      ifelse any? patches with [pcolor = t_inverse_color] in-radius 3
      [
        set energy energy - 1
        put-down-chip
      ]
