...
humans-own[job ctask]

globals[taux_hunt energy]

to setup
  __clear-all-and-reset-ticks
  set-default-shape turtles "person"
  set-default-shape animals "bug"

  initialize_village
  
  initialize_forest
  
  create-animals count_animals
  [
    set color yellow
    setxy random-xcor random-ycor
  ]

  create-humans count_humans / 2
  [
    set color red
    set job "hunter"
    set energy 100
  ...
  set taux_hunt energy_hunt / (count_humans / 2)
  
end

to go
  ask animals[go_animals]
  ask humans[run ctask]
...
to go_animals
 ...
 if random 100 < taux_reproduction
 [
   hatch 1 [ lt 45 fd 1 ]
 ]
end
...
to go_hunter
  ifelse random 100 > taux_hunt
  [ 
    hunt
  ]
  [
   set ctask "go_trapper" 
  ]
end

to initialize_village
  let start_width -2
  repeat 5
  [
    let start_height -2
    repeat 5
    [
      ask patch start_width start_height [set pcolor brown]
      set start_height start_height + 1
    ]
    set start_width start_width + 1
  ]
end
...
to initialize_forest
  ...
  [
    ...
    repeat 5
    [
      ask patch start_width start_height [set pcolor green]
      set start_height start_height + 1
    ]
    set start_width start_width + 1
  ]
...
to go_hunt
  ...
  let target one-of animals in-radius 5
  if target != nobody
  [
    set ctask "hunt"
  ]
...
to hunt 
  let target one-of animals in-radius 3
  ifelse target != nobody
  [
    face target
    fd 3
    ask target[die]
    set ctask "back_village"
  ]
  [
    set ctask "go_hunt"
  ]
...
to go_trapp
  ...
  if pcolor = green
  [
    face one-of patches in-radius 1 with [pcolor = green]
    fd 1
    set ctask "trapp"
  ]
end
...
to back_village
  ...
  [
    ifelse random 100 > (energy_hunt / (count_humans / 2))
    [
      set ctask "go_hunt"
    ]
    [
    ...
    ifelse job = "hunter"
    [
      set energy energy + energy_hunt
    ]
    [
      set energy energy + energy_trapp
    ]
  ]
end
@#$#@#$#@
